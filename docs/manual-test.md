...
- 作業ディレクトリ: `D:\AI\VSCodeExtension\ui-text-translation-key-manager`
...
```powershell
cd D:\AI\VSCodeExtension\ui-text-translation-key-manager
npm test
npm start
```
...
5. VS Code extension + key catalog の UI または配布面を開き、検証結果をユーザーが次アクションに移せる粒度で確認する。
...
- docs ZIP が `dist/ui-text-translation-key-manager-docs.zip` に存在する。
